...
public class IGA {
    ...
    public void initialize(DataSet dataSet, double mutationRate, int numberOfTournaments, int iterations){
        ...
        for(int i=0; i < numberOfIslands; i++){
            ...
            AIS ais = new AIS(dataSet.trainingSet,dataSet.featureMap,dataSet.labels,dataSet.antigenMap,dataSet.validationAntigenMap, (this.populationSize/this.numberOfIslands), mutationRate, numberOfTournaments, iterations,dataSet.featureSubsets);
        ...
        if(this.MASTERISLAND) {
             this.masterIsland = new MasterIsland(
                    new AIS(dataSet.trainingSet,dataSet.featureMap,dataSet.labels,dataSet.antigenMap,dataSet.validationAntigenMap, this.populationSize, mutationRate, numberOfTournaments, iterations,dataSet.featureSubsets),
                    migrationRate,
                    migrationFrequency,
                    this.islands
            );
